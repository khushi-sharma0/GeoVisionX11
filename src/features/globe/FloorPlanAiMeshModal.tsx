...
export const FloorPlanAiMeshModal: React.FC<FloorPlanAiMeshModalProps> = ({
  ...
  const activeUnitsList = (units && units.length > 0 ? units : [
    {
      ...
      ownerName: 'Bandra Commercial Holdings',
  ...
  // Extract authentic building footprint polygon
  ...
  const availH = 400 - padY * 2;

  const spanLng = Math.max(0.00002, (maxLng - minLng) * cosLat);
  const spanLat = Math.max(0.00002, maxLat - minLat);
  ...
  const handleExportDxf = () => {
    ...
    units.forEach((u, i) => {
      ...
      dxf += `0\nTEXT\n8\nUNIT_LABELS\n10\n${xM}\n20\n${yM}\n30\n0.0\n40\n2.5\n1\n${u.flatNumber || u.unitId} (${u.carpetAreaSqm.toFixed(0)} sqm)\n`;
    });
              ...
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">
                  {activeMode === 'floorPlan'
                    ...
                    : `3D AI Mesh Reconstruction — ${building.name}`}
                </h2>
          ...
          {activeMode === 'floorPlan' ? (
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center gap-2 text-slate-400 text-[11px] font-mono">
                  ...
                  <span>·</span>
                  <span>Gross Slab Area: {floorGrossArea} m²</span>
                  <span>·</span>
                  <span>Units: {units.length || 3}</span>
                </div>
              </div>
                  ...
                  {showDimensions && (
                    <g className="text-[10px] font-mono fill-slate-400">
                      ...
                      <line
                        ...
                        x2={centerSvgX + (spanLng * scale) / 2}
                        y2={centerSvgY - (spanLat * scale) / 2 - 20}
                        stroke="#64748b"
                        strokeWidth="1"
                        strokeDasharray="4 2"
                      />
                      <text
                        x={centerSvgX}
                        y={centerSvgY - (spanLat * scale) / 2 - 25}
                        textAnchor="middle"
                        fill="#38bdf8"
                      >
                        {widthM} meters (Width)
                      ...
                      <line
                        ...
                        x2={centerSvgX - (spanLng * scale) / 2 - 20}
                        y2={centerSvgY + (spanLat * scale) / 2}
                        stroke="#64748b"
                        strokeWidth="1"
                        strokeDasharray="4 2"
                      />
                      <text
                        x={centerSvgX - (spanLng * scale) / 2 - 26}
                        y={centerSvgY}
                        textAnchor="middle"
                        fill="#38bdf8"
                        transform={`rotate(-90 ${centerSvgX - (spanLng * scale) / 2 - 26} ${centerSvgY})`}
                      >
              ...
              <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-750 text-xs space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div className="p-2.5 rounded-lg bg-slate-800/80 border border-slate-700/60">
                    <div className="text-[11px] text-slate-400">Total Slab Footprint</div>
                    <div className="text-sm font-bold text-white font-mono mt-0.5">{floorGrossArea.toFixed(1)} m²</div>
                    <div className="text-[10px] text-slate-500">Gross Floor Plate (100%)</div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-slate-800/80 border border-slate-700/60">
                    ...
                    <div className="text-[10px] text-slate-500">Lifts, Stairs, Hallway (20%)</div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-slate-800/80 border border-slate-700/60">
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
                ...
                <div className="flex items-center gap-2 text-slate-400 text-[11px] font-mono">
                  <span>Vertices: 18,420</span>
                  <span>·</span>
                  <span>Faces: 36,280</span>
                  <span>·</span>
                  <span>LoD 3.2</span>
                </div>
              </div>
              ...
              <div className="relative w-full aspect-[16/9] bg-slate-950 border border-slate-800 rounded-xl overflow-hidden flex items-center justify-center p-6">
                ...
                <div className="absolute top-4 left-4 bg-slate-900/90 border border-slate-700/80 rounded-lg p-2.5 backdrop-blur-md text-xs space-y-1">
                  <div className="flex items-center gap-1.5 text-cyan-300 font-bold text-[11px]">
                    <Sparkles size={13} />
                    <span>Deep-Learning Mesh Extraction</span>
                  </div>
                  <div className="text-[10px] text-slate-400">
                    Source: Drone LiDAR + Google 3D Tiles
                  </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/60">
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Complete building exterior, window reveals, and core slab separation.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-xl bg-slate-800/50 border border-slate-700/60">
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Aligned with BMC development control regulations.
                  </p>
                </div>
              </div>
            </div>
